...
int main() 
{
	...
	//set shift on fake Cu insulator;
	const double Ef = 0.57553;
	double ins_shift = -0.2;
	// number of spacer layers
	int N = 10;
	// set bias
	double V = 0.05;
	//set number of insulator bilayers
	int lim = 3;//TODO remember in halite structures the two basis atoms are on the same plane
	...
	vector<double> Co1, Co2, Cu1, Cu2;//TODO recent addition (29-5-19) ins is sps+0.1 for 1st neighbour in Cu, so onsite and second nn are for Cu
	...
	Co_u = U(1,1);
	Cu = U(2,0);
	/* ins0 = Cu + ins_shift*I;//TODO 31-5-19 this shift places the fermi level in the artificial HG */
	...
	//This section defines the basis atoms and lattice vectors for RHS Co, Cu
	Vector3d LH_bas1, LH_bas2;
	vec3 LH_basis;
	LH_basis.reserve(2);//magic 2 is number of subatoms
	LH_bas1<< 0., 0., 0.;
	LH_bas2<< 0.5,-0.5, 0.;
	LH_basis.emplace_back(LH_bas1);
	LH_basis.emplace_back(LH_bas2);
	//This section defines the primitive lattice vectors
	Vector3d LH_lat_oop, LH_lat_ip1, LH_lat_ip2;
	LH_lat_oop << 0, 1, 0;
	LH_lat_ip1 << 0.5, 0,-0.5;
	LH_lat_ip2 << 0.5, 0, 0.5;
	//Distance information for n.n and n.n.n
	double LH_nn_dist, LH_nnn_dist;
	LH_nn_dist = M_SQRT2/2.;
	LH_nnn_dist = 1.;

	//This section defines the basis atoms and lattice vectors for RHS Co, Cu
	Vector3d RH_bas1, RH_bas2, tmp_vec;
	vec3 RH_basis;
	RH_basis.reserve(2);//magic 2 is number of subatoms
	RH_bas1<< 0., 0., 0.;
	RH_bas2<< 0.5, 0.5, 0.;
	RH_basis.emplace_back(RH_bas1);
	RH_basis.emplace_back(RH_bas2);
	//This section defines the primitive lattice vectors
	Vector3d RH_lat_oop, RH_lat_ip1, RH_lat_ip2;
	RH_lat_oop << 0, 1, 0;
	RH_lat_ip1 << 0.5, 0,-0.5;
	RH_lat_ip2 << 0.5, 0, 0.5;
	//Distance information for n.n and n.n.n
	double RH_nn_dist, RH_nnn_dist;
	RH_nn_dist = M_SQRT2/2.;
	RH_nnn_dist = 1.;

	//This section defines the basis atoms and lattice vectors for LiCl
	Vector3d ins_bas1, ins_bas2;
	vec3 ins_basis;
	ins_basis.reserve(2);//magic 2 is number of subatoms
	ins_bas1<< 0.5, 0., 0.;
	ins_bas2<< 0, 0., 0.5;
	ins_basis.emplace_back(ins_bas1);
	ins_basis.emplace_back(ins_bas2);
	//This section defines the primitive lattice vectors
	Vector3d ins_lat_oop, ins_lat_ip1, ins_lat_ip2;
	ins_lat_oop << 0.5, 0.5*M_SQRT2, 0.5;
	ins_lat_ip1 << 1, 0, 0;
	ins_lat_ip2 << 0, 0, 1;
	//Distance information for n.n and n.n.n
	double ins_nn_dist, ins_nnn_dist, ins_nnnn_dist;
	ins_nn_dist = M_SQRT2/2.;
	ins_nnn_dist = 1.;
	ins_nnnn_dist = sqrt(3.)/M_SQRT2;
	Vector3d ins_met_lat_oop;
	...
	RH_pos.reserve(19); LH_pos.reserve(19); ins_pos.reserve(19);
	...
	for (int i1 = -1; i1 < 2; i1++){
		for (int i2 = -1; i2 < 2; i2++){
			for (int i3 = -1; i3 < 2; i3++){
				for (int i4 = 0; i4 < RH_basis.size(); i4++){
					tmp_vec = i1*RH_lat_ip1 + i2*RH_lat_ip2 + i3*RH_lat_oop + RH_basis[i4];
					distance = 0;
					for (int l = 0; l < 3; l++)
						distance += tmp_vec(l)*tmp_vec(l);
					distance = sqrt(distance);
					...
					tmp_vec = i1*RH_lat_ip1 + i2*RH_lat_ip2 + i3*ins_met_lat_oop + RH_basis[i4] - ins_basis[i4];
					/* distance = 0; */
					/* for (int l = 0; l < 3; l++) */
					/* 	distance += tmp_vec(l)*tmp_vec(l); */
					/* distance = sqrt(distance); */
					...
					tmp_vec = i1*LH_lat_ip1 + i2*LH_lat_ip2 + i3*LH_lat_oop + LH_basis[i4];
					distance = 0;
					for (int l = 0; l < 3; l++)
						distance += tmp_vec(l)*tmp_vec(l);
					distance = sqrt(distance);
					...
					tmp_vec = i1*ins_lat_ip1 + i2*ins_lat_ip2 + i3*ins_lat_oop + ins_basis[i4];
					distance = 0;
					for (int l = 0; l < 3; l++)
						distance += tmp_vec(l)*tmp_vec(l);
					distance = sqrt(distance);
      ...
      atname.emplace_back("Li");
      atname.emplace_back("Cl");
...
//     whole cluster
	...
	for (int i3 = -5; i3 <= 3; i3++){
          for (int i1=-5; i1 <= 3; i1++){
            for (int i2= 0; i2 < RH_basis.size(); i2++){
		    if ((ilay > 1) && (ilay < 2 + lim))
              		rr= i1*ins_lat_ip1+i3*ins_lat_ip2 + ins_basis[i2] + counter;
		    else if (ilay < 2)
              		rr= i1*LH_lat_ip1+i3*LH_lat_ip2 + LH_basis[i2] + counter;
		    else
              		rr= i1*RH_lat_ip1+i3*RH_lat_ip2 + RH_basis[i2] + counter;
	...
	if (ilay == 1)
		counter = counter + ins_met_lat_oop;
	else if ((ilay > 1) && (ilay < 1 + lim))
		counter = counter + ins_lat_oop;
	else if (ilay == 1 + lim)
		counter = counter + ins_met_lat_oop;
	else
		counter = counter + RH_lat_oop; 
      }
