...
int main() 
{
	...
	int lim = 1;//TODO remember in halite structures the two basis atoms are on the same plane
	...
	for (int i1 = -1; i1 < 2; i1++){
		for (int i2 = -1; i2 < 2; i2++){
			for (int i3 = -1; i3 < 2; i3++){
				for (int i4 = 0; i4 < basis.size(); i4++){
					tmp_vec = i1*lat1 + i2*lat2 + i3*lat3 + basis[i4];
					distance = 0;
					for (int l = 0; l < 3; l++)
						distance += tmp_vec(l)*tmp_vec(l);
					distance = sqrt(distance);
