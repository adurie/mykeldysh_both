...
int main() 
{
	...
	//This section defines the basis atoms and lattice vectors for Cu
	basis[0][0] << 0., 0., 0.;
	...
	basis[1][0]<< 0.5, 0., 0.;//Li
	basis[1][1]<< 0.0, 0., 0.5;//Cl
	basis[1][2]<< 0.5, 0.5*M_SQRT2, 0.;//Cl
	basis[1][3]<< 0.0, 0.5*M_SQRT2, 0.5;//Li
	attype[1][0] = species(3);//3 is Li
	attype[1][1] = species(4);//4 is Cl 
	attype[1][2] = species(4);//4 is Cl 
	attype[1][3] = species(3);//3 is Li
	dist_tmp.emplace_back(M_SQRT2/2.);
	dist_tmp.emplace_back(1.);
	dist_tmp.emplace_back(sqrt(3.)/M_SQRT2);
	lat_oop[species(3)] << 0.0, M_SQRT2, 0;
	lat_oop[species(4)] << 0.0, M_SQRT2, 0;
	tmp_atom = species(3) + species(4);
	dist[tmp_atom] = dist_tmp;
	lat_oop[tmp_atom] << 0.0, M_SQRT2, 0;
	...
	lat_oop[species(1) + species(3)] << 0.5, 0.6393 + 0.5, 0.;// this from LiCl paper detailing distance between LiCl and Co/Cu, PRB 63 205418 (2.93A between Cl & nn Cu, Cu has a = 3.61A)
	lat_oop[species(1) + species(4)] << 0.5, 0.6393 + 0.5, 0.;// this from LiCl paper detailing distance between LiCl and Co/Cu, PRB 63 205418 (2.93A between Cl & nn Cu, Cu has a = 3.61A)
	lat_oop[species(3) + species(2)] << 0., 0.6393 + 0.5*M_SQRT2, 0.;// this from LiCl paper detailing distance between LiCl and Co/Cu, PRB 63 205418 (2.93A between Cl & nn Cu, Cu has a = 3.61A)
	lat_oop[species(4) + species(2)] << 0., 0.6393 + 0.5*M_SQRT2, 0.;// this from LiCl paper detailing distance between LiCl and Co/Cu, PRB 63 205418 (2.93A between Cl & nn Cu, Cu has a = 3.61A)
	lat_oop[species(1) + species(2)] << 0, 1., 0;
	...
	dist_tmp.emplace_back(1.2247);//or expansion at interfaces - 2nd nn
	ntype_tmp.emplace_back(1);
	ntype_tmp.emplace_back(2);
	ntype_tmp.emplace_back(3);
	...
	//Cu-Li, Cu-Cl 
	//TODO need to think about all the nearest neighbour distances here...
	dist_tmp.emplace_back(0.811606);
	dist_tmp.emplace_back(1.1393);
	dist_tmp.emplace_back(1.287907);
	/* dist_tmp.emplace_back(1.346407); */
	ntype_tmp.emplace_back(1);
	ntype_tmp.emplace_back(2);
	ntype_tmp.emplace_back(3);
	...
	oddattype[0][2] = species(1);//edit
	oddattype[0][3] = species(1);//edit
	oddattype[1][0] = species(1);//edit
	oddattype[1][1] = species(1);//edit
	oddattype[1][2] = species(2);//edit
	oddattype[1][3] = species(2);//edit
	oddattype[2][0] = species(2);//edit
	oddattype[2][1] = species(2);//edit
	oddattype[2][2] = species(2);//edit
	oddattype[2][3] = species(2);//edit
			...
			if (dist.count(tmp_atom)){
				for (int m = 0; m < dist[tmp_atom].size(); m++){
					...
					for (int kk = 0; kk < 10; kk++){
						...
						if ((tmpat1 == "Cu") && ((tmpat2 == "Li") || (tmpat2 == "Cl")))//atom names are explicit here as we only need to do this for this system
							tmp = gmean(hop_up[tmpat1][l][kk], hop_up[tmpat1][l][kk], dist[tmpat1][l], dist[tmpat1][l], dist[tmp_atom][m], order);
						else if ((tmpat2 == "Cu") && ((tmpat1 == "Li") || (tmpat1 == "Cl")))//atom names are explicit here as we only need to do this for this system
							tmp = gmean(hop_up[tmpat2][l][kk], hop_up[tmpat2][l][kk], dist[tmpat2][l], dist[tmpat2][l], dist[tmp_atom][m], order);
						else if ((tmpat1 == "Co") && ((tmpat2 == "Li") || (tmpat2 == "Cl")))
							tmp = gmean(hop_up[tmpat1][l][kk], hop_up[tmpat1][l][kk], dist[tmpat1][l], dist[tmpat1][l], dist[tmp_atom][m], order);
						else if ((tmpat2 == "Co") && ((tmpat1 == "Li") || (tmpat1 == "Cl")))
							tmp = gmean(hop_up[tmpat2][l][kk], hop_up[tmpat2][l][kk], dist[tmpat2][l], dist[tmpat2][l], dist[tmp_atom][m], order);
						else
