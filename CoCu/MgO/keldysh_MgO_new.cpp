...
int main() 
{
	...
	const int lim = 2;//remember in halite structures the two basis atoms are on the same plane
	...
	tmpbas1<< 0., 0., 0.;
	tmpbas2<< 0.5, 0.5, 0.;
	tmp_basis.emplace_back(tmpbas1);
	tmp_basis.emplace_back(tmpbas2);
	basis.emplace_back(tmp_basis);
	tmp_atom = species(1);//1 is Au
	tmp_at.emplace_back(tmp_atom);
	tmp_at.emplace_back(tmp_atom);
	attype.emplace_back(tmp_at);
	tmp_lat_oop << 0, 1, 0;
	lat_oop[tmp_atom] = tmp_lat_oop;
	...
	//TODO the below... be aware also that I need to code in the second Au and Fe in attype and probably others...
	//TODO also, consider whether we need to do the whole 5-layer structure in the massive loop below
	  ...
	  for (int inter = 0; inter < 2; inter++){
	    ...
	    for (int ii = 0; ii < numbas; ii++){
	      for (int jj = 0; jj < numbas; jj++){
			...
			atomtype = attype[at][ii] + attype[at - inter][jj];

		      //TODO build case where if e.g. second Au (or Fe) then skip based on comparing keys
	              tmp_vec = i1*lat_vec1 + i2*lat_vec2 + i3*lat_oop[atomtype] + basis[at][ii] - basis[at - inter][jj];//TODO verify the order of ii, jj
		      //TODO if i1 = i2 = i3 = start, see test.cpp
	              distance = 0;
	              for (int l = 0; l < 3; l++)
	                distance += tmp_vec(l)*tmp_vec(l);
	              distance = sqrt(distance);
	              //TODO be wary about MgO!
		      //perhaps utilise map of key string where container like attype[at][ii] = string so compare attype[at][ii] vs attype[at - inter][jj]
		      //then rework a great deal of below to accommodate this
	              if ((inter == 0) && (distance < 1e-5)){
			if ((i1 == i1start) && (i2 == i2start) && (i3 == i3start)){
	                  pos[atomtype][ii][jj][0] = tmp_vec;//TODO verify the order of ii, jj
	                  atom_up[atomtype][ii][jj][0] = onsite_up[atomtype];//TODO verify the order of ii, jj
	                  atom_dn[atomtype][ii][jj][0] = onsite_dn[atomtype];//TODO verify the order of ii, jj
			}
			else {
	                  pos[atomtype][ii][jj].emplace_back(tmp_vec);//TODO verify the order of ii, jj
	                  atom_up[atomtype][ii][jj].emplace_back(onsite_up[atomtype]);//TODO verify the order of ii, jj
	                  atom_dn[atomtype][ii][jj].emplace_back(onsite_dn[atomtype]);//TODO verify the order of ii, jj
			}
	              }
			    ...
			    if ((i1 == i1start) && (i2 == i2start) && (i3 == i3start)){
	                      pos[atomtype][ii][jj][0] = tmp_vec;//TODO verify the order of ii, jj
	                      atom_up[atomtype][ii][jj][0] = tmp_mat_up;//TODO verify the order of ii, jj
	                      atom_dn[atomtype][ii][jj][0] = tmp_mat_dn;//TODO verify the order of ii, jj
			    }
			    else {
	                      pos[atomtype][ii][jj].emplace_back(tmp_vec);//TODO verify the order of ii, jj
	                      atom_up[atomtype][ii][jj].emplace_back(tmp_mat_up);//TODO verify the order of ii, jj
	                      atom_dn[atomtype][ii][jj].emplace_back(tmp_mat_dn);//TODO verify the order of ii, jj
			    }
	                    break;
			  }
	                }
