...
typedef struct
	{
		...
		unordered_map<double, vector<double>> all_the_Es;
		vector<bool> *isMag;
...
M9 InPlaneH(const vec3 &pos, const Vector3d &basis, const vM &U, const double x, const double z){
 ...
 vector<double> f(const double theta, const dcomp E, variables * send, const MatrixXcd &GR_UP_even,  
...
vector<double> int_theta_E(const dcomp E, variables * send) { 
 	double theta; 
 	int N = send->N2; 
	int numbas = send->numbas;
	int n = numbas*9;
 ...
 //build thickness of layer 3 to lim2 layers 
	...
	vector<double> integrate, result;
	for (int kk = 0; kk < N; kk++)
 	...
 	for (int kk=0; kk<nn+1; kk++) { 
 		theta = kk*M_PI/nn; 
 		integrate = f(theta, E, send, GR_up_even, GR_dn_even, GR_up_odd, GR_dn_odd, GL_up, GL_dn); 
 		if ((kk==0)||(kk==nn)){ 
			for (int ll = 0; ll < N; ll++)
 				result[ll] += M_PI*(0.5/nn)*integrate[ll]; 
 		} 
 		else { 
			for (int ll = 0; ll < N; ll++)
 				result[ll] += (M_PI/nn)*integrate[ll]; 
 		} 
 	} 	
	return result;
 } 

void pass(const double E[], long nx, double fv[], long *iflag, Nag_Comm * comm) {
	variables * send = (variables *) comm->p;
	int i = send->it2;
	dcomp E_send;
	dcomp im = -1;
	...
	for (int k = 0; k < nx; k++){
		tmp = E[k];
		if (!send->all_the_Es.count(tmp)){
			E_send = tmp + 1e-6*im;//TODO Andrey has 1e-8 here
			send->all_the_Es.emplace(tmp, int_theta_E(E_send, send));
			fv[k] = send->all_the_Es[tmp][i];
		}
		else {
			fv[k] = send->all_the_Es[tmp][i];
		}
...
vector<double> int_energy(variables * send) {
	...
	NagError status;
	INIT_FAIL(status);
	send->all_the_Es.clear();
	for (int it = N - 1; it > -1; it--){
		send->N2 = it + 1;
		send->it2 = it;
		comm.p = send;
		d01rgc(a, b, pass, tol, tol, &result, &errest, &n_evals, &comm, &status);
 		if (status.code != NE_NOERROR)
 			cout<<"For x = "<<send->x<<", z = "<<send->z<<" "<<status.message<<endl<<endl;
		result_vec[it] = result;
	}
...
int main() 
{
	...
	const int N = 10;
	const int numblocks = 3;//setting the number of partitions of the irreducible segment
	// set bias
	const double V = 0.05;
