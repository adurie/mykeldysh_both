...
//adlayer layer 2 from layer 1 to spacer thickness, N
	...
	for (int it=0; it < N-1; ++it){
		...
		if (myswitch == 0){
			...
			spincurrent = (1./(2.*M_PI))*real(TOT.trace()*(fermi(E,Ef+V)-fermi(E,Ef)));
		}
		if (myswitch == 1){
			TOT = (B - A)*Pauli;
...
/* 	/1* gsl_integration_workspace * w = gsl_integration_workspace_alloc (1000); *1/ */
/* 	/1* double error, ans; *1/ */
/* 	/1* gsl_function F; *1/ */
/* 	/1* F.function = &pass; *1/ */
/* 	/1* vector<double> args; *1/ */
/* 	/1* args.emplace_back(x); *1/ */
/* 	/1* args.emplace_back(z); *1/ */
/* 	/1* args.emplace_back(a); *1/ */
/* 	/1* args.emplace_back(Ef); *1/ */
/* 	/1* args.emplace_back(N); *1/ */
/* 	/1* vector<vector<double>> hold_all; *1/ */
/* 	/1* hold_all.emplace_back(result); *1/ */
/* 	/1* hold_all.emplace_back(args); *1/ */
/* 	/1* F.params = &hold_all; *1/ */
/* 	/1* gsl_integration_qags (&F, start, end, 0, 1e-2, 1000, w, &ans, &error); *1/ */
/* 	/1* gsl_integration_workspace_free (w); *1/ */
...
vector<double> switching(double x, double z, double a, double Ef, int N, double theta){
	...
	for (int j=0; j!=10; j++){
		...
		integrate = f(x, z, a, E, Ef, N, theta, 1);
		for (int l = 0; l < N; l++)
...
vector<double> int_theta(double x, double z, double a, double Ef, int N){
	vector<double> result;
	vector<double> integrate;
	result.reserve(N);
	integrate.reserve(N);
	...
	for (int k=0; k<n+1; k++) {
		theta = k*M_PI/n;
		integrate = switching(x, z, a, Ef, N, theta);
		for (int i = 0; i < N; i++){
			if ((k==0)||(k==n))
				result[i] += M_PI*(0.5/n)*integrate[i];
			else 
...
/* 					counter++; */
/* 					z = M_PI*l/n; */
/* 					/1* integrate = int_theta(x, z, 1, Ef, Ef, N); *1/ */
/* 					/1* integrate = int_energy(x, z, 1, Ef, N); *1/ */
