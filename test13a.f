...
c     PROGRAM jmbcocu.f
c
c     calculates spin-current of
c     ... Cu | MgO(nins) | Co(mlay) | Cu(nlay) | Co ...
c
c
c            |\
c            | \
...
c     
      ...
      parameter (nplx=20,nslx=1,nspinx=1)
...
c                                    npmax - max No of points per plane
      parameter (lwx=1000,liwx=1000)
      dimension w(lwx),iw(liwx)

      complex*16 ec
      common/fermi/efl,efr
      common/lbl/ pi, sq2, sq3
      common/data/frat,ifrac,nmat,nspin,nsl,mlay,nins
      common/layer/nmin,ndiff
      common/geom/a1(3),a2(3),a3(3),xn(3),numnn 
      common/datfunc/ec,b1(3),b2(3),nq,ntheta,irecip,in
...
c     fraction of an atomic layer to be adlayered
      ...
      nmin=0
      nmax=10      !!! Max No of Cu layers
      mlay=10       !!! No of Co principal layers on LHS
      nins=5       !!! No of insulating layers
      ndiff=(nmax-nmin)*ifrac
      nmat=nsl*nspin

      vbias=0.0d0
      efl=0.d0        !! LH Fermi level
      efr=efl-vbias        !! RH Fermi level
      nq=1
      ntheta= 10
      tfac  = 8.617d-05/ryd
      temp  = 300.d0*tfac
...
CCC   call recip(xn,b1,b2,irecip)
      call recip1(a1,a2,a3,xn,numnn,b1,b2,irecip)
...
CCC      emin=-3.d0
CCC      emax=0.5d0
CCC      de=(emax-emin)/nen
CCC      sp=0.d0
CCCC     Trapezium rule to integrate w.r.t E from emin to emax
CCC      do ifoo=0,nen
CCC        er=emin+ifoo*de
CCC        ec=dcmplx(er,wm)
CCC
CCCCCC     write(*,*)' Energy =',ec
CCC        do in=0,ndiff
CCC          vruu(in)=0.d0
CCC        enddo
CCC        call sumk(irecip,ec,nq,ntheta,b1,b2)
CCC
CCC        ifac=2
CCC        if(ifoo.eq.0.or.ifoo.eq.nen)ifac=1
CCC        do in=0,ndiff
CCC          vcuu(in) = vcuu(in)+0.5d0*ifac*de*vruu(in)
CCC        enddo
CCC
CCCCCC     write(*,*)er,vruu(4)
CCC      enddo
      write(*,*)
      write(*,*)' INTEGRATED COUPLING'
      write(*,*)
...
CCC        er=emin+ifoo*de
CCC        in=4
CCC        write(33,*)er,func(er)
CCC      enddo


      do in=1,ndiff
        call d01ajf(func,emin,emax,eps,eps,res,abser,w,lwx,iw,
     $liwx,ifail)
        write(*,*)in,res,iw(1)
      enddo
...
c
     ...
     $  vrdu(0:nlayx)
      ...
      wm = 1.d-6
      ec=dcmplx(x,wm)
      call sumk(irecip,ec,nq,ntheta,b1,b2)
      func=vruu(in)
      return
      end
c
c     *****************************************************************
c
      subroutine sumk(irecip,ec,nk,ntheta,b1,b2)
      implicit double precision (a-h,o-y)
      implicit complex*16 (z)
      parameter (nplx=20,nslx=1,nspinx=1)
      parameter (nmatx=nslx*nspinx)
...
c                                    npmax - max No of points per plane
      ...
      dimension wkspc(n2x,n2x),wkspc2(n2x)
      dimension xk(3),indx(n2x)
      complex*16 wkspc
      common/lbl/ pi, sq2, sq3
      common/data/frat,ifrac,nmat,nspin,nsl,mlay,nins
      common/layer/nmin,ndiff
      ...
      common/structure/rp(-nslx:nslx,npmax,3),cosp(-nslx:nslx,npmax,3),
     $  np(-nslx:nslx),nnp(-nslx:nslx,npmax),ipmax
      dimension tquu(0:nlayx)
      common/parts/vquu(0:nlayx),vqdd(0:nlayx),vqud(0:nlayx),
     $  vqdu(0:nlayx)
      common/chem/temp
      common/fermi/efl,efr
      common/bias/vbias
      dimension zpx2(n2x,n2x),zpy2(n2x,n2x)
      dimension zs2(n2x,n2x),zs2inv(n2x,n2x)
      dimension zt2(n2x,n2x),zt2dag(n2x,n2x)
      dimension zgl2(n2x,n2x),zgr2(n2x,n2x)
      dimension ztmp2(n2x,n2x),zlr2(n2x,n2x),zrl2(n2x,n2x)
      dimension zfoo1(n2x,n2x),zfoo2(n2x,n2x),zfoo3(n2x,n2x)
      dimension zfoo4(n2x,n2x)
c
c
...
c     -----------------------------------------------------------------
C     Now make (2nmat)x(2nmat) Hopping Matrices
      do ir=1,nmat
        do is=1,nmat
          zt2(ir,is)=zt(ir,is)
          zt2(ir+nmat,is+nmat)=zt(ir,is)
          zt2(ir,is+nmat)=0.d0
          zt2(ir+nmat,is)=0.d0
        enddo
      enddo
      do ir=1,nmat2
        do is=1,nmat2
          zt2dag(ir,is)=dconjg(zt2(is,ir))
        enddo
      enddo
c     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C     Define the Pauli Sigma Matrix
      do ir=1,nmat2
        do is=1,nmat2
          zpy2(ir,is)=0.d0
        enddo
      ...
      do ir=1,nmat
        zpy2(ir,ir)=0.d0
        zpy2(ir+nmat,ir+nmat)=0.d0
        zpy2(ir,ir+nmat)=dcmplx(0.d0,-1.d0)
        zpy2(ir+nmat,ir)=dcmplx(0.d0,1.d0)

        zpx2(ir,ir)=0.d0
        zpx2(ir+nmat,ir+nmat)=0.d0
        zpx2(ir,ir+nmat)=dcmplx(1.d0,0.d0)
        zpx2(ir+nmat,ir)=dcmplx(1.d0,0.d0)
      enddo
c     -----------------------------------------------------------------
C     Do the integratration with respect to theta
      do ii=0,ndiff
        vquu(ii)=0.d0
      enddo
      do itheta=0,ntheta
...
C       Define the rotation matrix zs2(theta)
        do ir=1,nmat2
          do is=1,nmat2
            zs2(ir,is)=0.d0
          enddo
        enddo
        do ir=1,nmat
          zs2(ir,ir)=cos(theta/2.d0)
          zs2(ir+nmat,ir+nmat)=cos(theta/2.d0)
          zs2(ir,ir+nmat)=sin(theta/2.d0)
          zs2(ir+nmat,ir)=-sin(theta/2.d0)
        enddo

        do ir=1,nmat2
          do is=1,nmat2
            zs2inv(ir,is)=zs2(ir,is)
          enddo
        enddo
        call invers(zs2inv,nmat2,n2x)
c       -----------------------------------------------------------------
c
c
...
c         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          ...
          enddo
c         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C         Define the 36x36 SGFs
          do ir=1,nmat
            do is=1,nmat
              zgl2(ir,is)=zglu(ir,is)
              zgl2(ir+nmat,is+nmat)=zgld(ir,is)
              zgl2(ir+nmat,is)=0.d0
              zgl2(ir,is+nmat)=0.d0

              zgr2(ir,is)=garu(ir,is,in)
              zgr2(ir+nmat,is+nmat)=gard(ir,is,in)
              zgr2(ir+nmat,is)=0.d0
              zgr2(ir,is+nmat)=0.d0
            enddo
          enddo
c         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c         ROTATE THE RH SGF
          call multiply(zs2inv,zgr2,ztmp2,nmat2,n2x)
          call multiply(ztmp2,zs2,zgr2,nmat2,n2x)
c         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C         EVALUATE THE SINGLE DENOMINATOR FORMULA
          call multiply(zgr2,zgl2,zrl2,nmat2,n2x)
          call multiply(zgl2,zgr2,zlr2,nmat2,n2x)
          do ir=1,nmat2
            zrl2(ir,ir)=zrl2(ir,ir)-1.d0   !!! note the wrong sign
            zlr2(ir,ir)=zlr2(ir,ir)-1.d0
          enddo
          do ir=1,nmat2
            do is=1,nmat2
              zrl2(ir,is)=-zrl2(ir,is)     !!! sign is corrected
              zlr2(ir,is)=-zlr2(ir,is)     !!! sign is corrected
            enddo
          enddo
          call invers(zlr2,nmat2,n2x)  !! is B^{\dag}  (spincurrent paper)
          call invers(zrl2,nmat2,n2x)  !! is A
          do ir=1,nmat2
            do is=1,nmat2
              ztmp2(ir,is)=zrl2(ir,is)-zlr2(ir,is)
            enddo
          enddo
          call multiply(ztmp2,zpy2,zfoo1,nmat2,n2x)
          zsumsy=0.d0
          do ir=1,nmat2
            zsumsy=zsumsy+zfoo1(ir,ir)   !! tr (A-B*)\sigma_y
          enddo
          xjsy=-dreal(zsumsy)/(4*pi)  !!  (1/4pi) Re tr (B-A)\s_y
c         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          xj0y=xjsy

          xjy=xj0y*(zfermi(ec-efl)+zfermi(ec-efr))

          tquu(in)=xjy

CCCC      change the dreal to a dimag for Matsubara frequencies ???
CCCC      tquu(in)=2*dimag(zsum)/(4*pi)
c         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c         
10      continue     
        ...
        if((itheta.eq.0).or.(itheta.eq.ntheta))then
          do ii=0,ndiff
            vquu(ii)=vquu(ii)+tquu(ii)
          enddo
        else
          do ii=0,ndiff
            vquu(ii)=vquu(ii)+2*tquu(ii)
          enddo
        endif
      enddo
      do ii=0,ndiff
        vquu(ii)=dt*vquu(ii)/2
      enddo
      return
      end
c
c     *****************************************************************
c
      complex*16 function zfermi(zen)
      implicit double precision (a-h,o-y)
      implicit complex*16 (z)
      common/chem/temp
      if(abs(temp).ge.1.d-8)zfermi=1.d0/(1-cdexp(zen/temp))
      if(abs(temp).lt.1.d-8)then
        if(dreal(zen).gt.0)zfermi=0.d0
        if(dreal(zen).le.0)zfermi=1.d0
      endif
      return
      end
c
...
c     and the RH surface GF for all principal layers
      ispin=1
      call green(ispin,zener,xk,zglu2,zgru)
c
c     -----------------------------------------------------------------
...
c     and the RH surface GF for all principal layers
      ispin=2
      call green(ispin,zener,xk,zgld2,zgrd)
c     -----------------------------------------------------------------
C     Calculate hopping for cleavage plane !!! WILL NEED TO INCLUDE BIAS!!!
      call hamil(zu1,zt1,xk,3)
c     -----------------------------------------------------------------
      return
      end
c
c     *****************************************************************
c
      subroutine green(ispin,zener,xk,zsurfl,zgr)
...
c     and the RH surface GF for all principal layers
      ...
      dimension zu_nm(nmatx,nmatx),zt_nm(nmatx,nmatx)
      dimension zu_fm(nmatx,nmatx),zt_fm(nmatx,nmatx)
      dimension zu_ins(nmatx,nmatx),zt_ins(nmatx,nmatx)
      dimension zur(nmatx,nmatx)
      dimension ztdag(nmatx,nmatx)
      dimension zsurfl(nmatx,nmatx),zsurfr(nmatx,nmatx)
      dimension ztemp(nmatx,nmatx)
      dimension xk(3)
      common/data/frat,ifrac,nmat,nspin,nsl,mlay,nins
      common/layer/nmin,ndiff
      common/lbl/pi,sq2,sq3
...
C     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C     Will need care here...RHS to drop by Vbias, Ins to drop linearly!!!
      call hamil(zu_fm,zt_fm,xk,ispin)   !! FM
      call hamil(zu_nm,zt_nm,xk,3)   !! NM
      call hamil(zu_ins,zt_ins,xk,4)   !! Ins
C     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c     *****************************************************************
c     evaluate LH SGF
      call surfacenew(zu_fm,zt_fm,zener,zsurfl,zsurfr)
      do ir=1,nmat
        do is=1,nmat
      ...
      do ir=1,nmat
        do is=1,nmat
          ztdag(is,ir)=dconjg(zt_nm(ir,is))
        enddo
      enddo

      do k=1,ndiff
        call adlayer1(zsurfr,zu_nm,ztdag,zener)   !! - gl(1)
        do ir=1,nmat
          do is=1,nmat
            zgr(ir,is,k)=zsurfr(ir,is)
          enddo
        enddo
      enddo
c     -----------------------------------------------------------------
...
c     *****************************************************************
c
      subroutine recip1(a1,a2,a3,xn,numnn,b1,b2,irecip)
      implicit double precision (a-h,o-y)
      implicit complex*16 (z)
      parameter (nplx=20,nslx=1,nspinx=1)
      parameter (nlayx=nplx*nslx)
...
c     first construct a lattice cluster
      icnt=0
      do i=-numnn,numnn,1
        do j=-numnn,numnn,1
          do k=-numnn,numnn,1
            ...
            do ll=1,3
              ...
              r(icnt,ll)=v(ll)
            enddo
              dv(icnt)=sqrt(v(1)**2 + v(2)**2 + v(3)**2)
          enddo
        enddo
      enddo
      ntot=icnt
      if(ntot.gt.lattx)then
        write(*,*)' ERROR - RECIP1 : ntot > lattx !! '
        stop
      endif
c     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c     sort into distance from origin
      call sort(dv,iorder,ntot)
      do i=1,ntot
        do j=1,3
          rtmp(i,j)=r(iorder(i),j)
        enddo
...
c     plane (with smallest angle between them)
      ...
      do i=2,ntot
        do l=1,3
          d(l)=rtmp(i,l)
        enddo
        if(abs(dot(d,xn,3)).lt.1.d-8)then
          an1(1)=d(1)
          an1(2)=d(2)
          an1(3)=d(3)
          ilast=i
      ...
      enddo
      write(*,*)' ERROR RECIP1 : cant find 1st in-plane lattice site'
...
5020  continue
      do i=ilast+1,ntot
        do l=1,3
          d(l)=rtmp(i,l)
        enddo
        if(abs(dot(d,xn,3)).lt.1.d-8)then   !!! d is in-plane
      ...
      enddo
      write(*,*)' ERROR RECIP1 : cant find 2nd in-plane lattice site'
      write(*,*)' try increasing numnn1 '
      stop
5030  continue
      ...
      if((an1an1.lt.1.d-8).or.(an2an2.lt.1.d-8))then
        write(*,*)' ERROR : RECIP1 : an1 or an2 = 0 !!',an1an1,an2an2
        write(*,*)' Try increasing numnn1 !! '
      endif
...
c
      ...
      if(abs(evln1-evln).lt.1.d-12)then
...
CCC     write(*,*)ir,evlab(ir),evlphi(ir)
        ...
        if(cdabs(ztemp-zevl(ir)).gt.1.d-12)then
...
c     define -pi < phi < pi
      do ir=1,nmat2
        ...
        xi=dimag(zevl(ir))
        evlab(ir)=cdabs(zevl(ir))
        if((xr.gt.0).and.(xi.ge.0))evlphi(ir)=atan(xi/xr)
        if((xr.gt.0).and.(xi.le.0))evlphi(ir)=atan(xi/xr)
        if((xr.lt.0).and.(xi.ge.0))evlphi(ir)=atan(xi/xr)+pi
        if((xr.lt.0).and.(xi.le.0))evlphi(ir)=atan(xi/xr)-pi
        if((xr.eq.0).and.(xi.ge.0))evlphi(ir)=pi/2.d0
        if((xr.eq.0).and.(xi.le.0))evlphi(ir)=-pi/2.d0
CCC     write(*,*)ir,evlab(ir),evlphi(ir)
        arg=evlphi(ir)
        ztemp=dcmplx(cos(arg),sin(arg))*evlab(ir)
        if(cdabs(ztemp-zevl(ir)).gt.1.d-12)then
